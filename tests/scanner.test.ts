...
function makeMockRpc(): RpcClient {
  return {
    async getNftApprovalForAllLogs() {
      return [];
    },
...
describe("auditWalletApprovals", () => {
  ...
  it("discovers NFT ApprovalForAll operator approvals and returns revoke tx data", async () => {
    const nftCollection = "0x2222222222222222222222222222222222222222";
    const rpc = makeMockRpc();
    rpc.getApprovalLogs = async () => [];
    rpc.getNftApprovalForAllLogs = async () => [
      {
  ...
  it("deduplicates NFT operator approvals and keeps the latest event", async () => {
    ...
    rpc.getApprovalLogs = async () => [];
    rpc.getNftApprovalForAllLogs = async () => [
      { tokenAddress: nftCollection, spender, blockNumber: 100n, transactionHash: "0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa" },
      { tokenAddress: nftCollection, spender, blockNumber: 200n, transactionHash: "0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb" },
    ];
